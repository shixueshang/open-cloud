...
import com.opencloud.base.provider.mapper.BaseDeveloperMapper;
...
public class BaseDeveloperServiceImpl extends BaseServiceImpl<BaseDeveloperMapper, BaseDeveloper> implements BaseDeveloperService {
    ...
    private final String ACCOUNT_DOMAIN = BaseConstants.ACCOUNT_DOMAIN_DEVELOPER;
     ...
     */
    @Override
    public void addUser(BaseDeveloper baseDeveloper, Integer status) {
        ...
        //默认注册用户名账户
        baseAccountService.register(baseDeveloper.getUserId(), baseDeveloper.getUserName(), baseDeveloper.getPassword(), BaseConstants.ACCOUNT_TYPE_USERNAME, status, ACCOUNT_DOMAIN, null);
        if (StringUtils.matchEmail(baseDeveloper.getEmail())) {
            //注册email账号登陆
            baseAccountService.register(baseDeveloper.getUserId(), baseDeveloper.getEmail(), baseDeveloper.getPassword(), BaseConstants.ACCOUNT_TYPE_EMAIL, status, ACCOUNT_DOMAIN, null);
        }
        if (StringUtils.matchMobile(baseDeveloper.getMobile())) {
            //注册手机号账号登陆
            baseAccountService.register(baseDeveloper.getUserId(), baseDeveloper.getEmail(), baseDeveloper.getPassword(), BaseConstants.ACCOUNT_TYPE_MOBILE, status, ACCOUNT_DOMAIN, null);
        }
    }
     ...
     */
    @Override
    public void updateUser(BaseDeveloper baseDeveloper, Integer status) {
        if (baseDeveloper == null || baseDeveloper.getUserId() == null) {
            return;
        }
        if (status != null) {
            baseAccountService.updateStatusByUserId(baseDeveloper.getUserId(), ACCOUNT_DOMAIN, status);
        }
    ...
    public UserAccount login(String account) {
        ...
        if (StringUtils.isNotBlank(thirdParty)) {
            ...
            //用户名登录
            baseAccount = baseAccountService.getAccount(account, thirdParty, BaseConstants.ACCOUNT_TYPE_USERNAME);

            // 手机号登陆
            if (StringUtils.matchMobile(account)) {
                baseAccount = baseAccountService.getAccount(account, thirdParty, BaseConstants.ACCOUNT_TYPE_MOBILE);
            }
            // 邮箱登陆
            if (StringUtils.matchEmail(account)) {
                baseAccount = baseAccountService.getAccount(account, thirdParty, BaseConstants.ACCOUNT_TYPE_EMAIL);
            }
        }
