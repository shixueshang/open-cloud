...
-- ----------------------------
--DROP TABLE base_action;
create sequence base_action_seq;
CREATE TABLE base_action (
  action_id number(20) NOT NULL ,--COMMENT '资源ID',
  action_code varchar2(100)  NOT NULL ,--COMMENT '资源编码',
  action_name varchar2(100)  NOT NULL ,--COMMENT '资源名称',
  action_desc varchar2(100)  DEFAULT NULL ,--COMMENT '资源描述',
  menu_id number(20) DEFAULT NULL ,--COMMENT '资源父节点',
  priority number(10) DEFAULT '0' NOT NULL ,--COMMENT '优先级 越小越靠前',
  status number(3) DEFAULT '1' NOT NULL ,--COMMENT '状态:0-无效 1-有效',
  create_time date NOT NULL,
  update_time date DEFAULT NULL,
  is_persist number(3) DEFAULT '0' NOT NULL ,--COMMENT '保留数据0-否 1-是 不允许删除',
  service_id varchar2(100)  NOT NULL ,--COMMENT '服务名称',
  PRIMARY KEY (action_id)
);
create index idx_base_action_code on base_action(action_code);
--create index idx_base_action_id on base_action(action_id);
comment on table base_action
  is '系统资源-功能操作';
-- Add comments to the columns
comment on column base_action.action_id
  is '资源ID';
comment on column base_action.action_name
  is '资源编码';
comment on column base_action.action_desc
  is '资源描述';
comment on column base_action.menu_id
  is '资源父节点';
comment on column base_action.priority
  is '优先级 越小越靠前';
comment on column base_action.status
  is '状态:0-无效 1-有效';
comment on column base_action.create_time
  is '创建时间';
comment on column base_action.update_time
  is '最后修改时间';
comment on column base_action.is_persist
  is '保留数据0-否 1-是 不允许删除';
comment on column base_action.service_id
  is '服务名称';
-- ----------------------------
...
-- ----------------------------
--DROP TABLE base_api;
create sequence base_api_seq;
CREATE TABLE base_api (
      api_id number(20) NOT NULL ,--COMMENT '接口ID',
      api_code varchar2(255)  NOT NULL ,--COMMENT '接口编码',
      api_name varchar2(100)  NOT NULL ,--COMMENT '接口名称',
      api_category varchar2(20)  DEFAULT 'default' ,--COMMENT '接口分类:default-默认分类',
      api_desc varchar2(100)  DEFAULT NULL ,--COMMENT '资源描述',
      request_method varchar2(255)  DEFAULT NULL ,--COMMENT '请求方式',
      content_type varchar2(255)  DEFAULT NULL ,--COMMENT '响应类型',
      service_id varchar2(100)  NOT NULL ,--COMMENT '服务ID',
      path varchar2(200)  DEFAULT NULL ,--COMMENT '请求路径',
      priority number(20) DEFAULT '0' NOT NULL  ,--COMMENT '优先级',
      status number(3) DEFAULT '1' NOT NULL  ,--COMMENT '状态:0-无效 1-有效',
      create_time date NOT NULL,
      update_time date DEFAULT NULL,
      is_persist number(3) DEFAULT '0' NOT NULL  ,--COMMENT '保留数据0-否 1-是 不允许删除',
      is_auth number(3) DEFAULT '1' NOT NULL  ,--COMMENT '是否需要认证: 0-无认证 1-身份认证 默认:1',
      is_open number(3) DEFAULT '1' NOT NULL  ,--COMMENT '是否公开: 0-内部的 1-公开的',
      class_name varchar2(255)  DEFAULT NULL ,--COMMENT '类名',
      method_name varchar2(255)  DEFAULT NULL ,--COMMENT '方法名',
      PRIMARY KEY (api_id),
      CONSTRAINT uniq_base_api_code UNIQUE (api_code)
);
create index idx_base_service_id on base_api(service_id);
--create index idx_base_api_id on base_api(action_id);
comment on table base_api
  is '系统资源-API接口';
-- Add comments to the columns
comment on column base_api.api_id
  is '接口ID';
comment on column base_api.api_code
  is '接口编码';
comment on column base_api.api_name
  is '接口名称';
comment on column base_api.api_category
  is '接口分类:default-默认分类';
comment on column base_api.api_desc
  is '资源描述';
comment on column base_api.request_method
  is '请求方式';
comment on column base_api.content_type
  is '响应类型';
comment on column base_api.service_id
  is '服务ID';
comment on column base_api.path
  is '请求路径';
comment on column base_api.priority
  is '优先级';
comment on column base_api.status
  is '状态:0-无效 1-有效';
comment on column base_api.is_persist
  is '保留数据0-否 1-是 不允许删除';
comment on column base_api.is_auth
  is '是否需要认证: 0-无认证 1-身份认证 默认:1';
comment on column base_api.is_open
  is '是否公开: 0-内部的 1-公开的';
comment on column base_api.status
  is '状态:0-无效 1-有效';
comment on column base_api.class_name
  is '类名';
comment on column base_api.method_name
  is '方法名';
...
INSERT INTO base_api VALUES ('1131753774187180033', '9f7ad288b3ebf970ced6a71b9d85fa67', 'GatewayServiceClient.getApiRateLimitList', 'default', '', 'GET', '', 'opencloud-base-provider', '/gateway/api/rateLimit', '0', '1', '2019-05-24 02:48:21', '2019-06-10 18:00:04', '1', '0', '0', 'GatewayServiceClient', 'getApiRateLimitList');
INSERT INTO base_api VALUES ('1131753774300426242', '1cfb2e64efc00a7617801597f267eb91', 'GatewayServiceClient.getApiBlackList', 'default', '', 'GET', '', 'opencloud-base-provider', '/gateway/api/blackList', '0', '1', '2019-05-24 02:48:21', '2019-06-10 17:59:55', '1', '0', '0', 'GatewayServiceClient', 'getApiBlackList');
INSERT INTO base_api VALUES ('1131753774426255362', '8682df9dce9d7adb63ca1b77d3826dd7', 'GatewayServiceClient.getApiRouteList', 'default', '', 'GET', '', 'opencloud-base-provider', '/gateway/api/route', '0', '1', '2019-05-24 02:48:21', '2019-06-10 18:00:51', '1', '0', '0', 'GatewayServiceClient', 'getApiRouteList');
INSERT INTO base_api VALUES ('1131753774677913601', 'be28403a54c45e3905ad9c6c0916e1da', 'GatewayServiceClient.getApiWhiteList', 'default', '', 'GET', '', 'opencloud-base-provider', '/gateway/api/whiteList', '0', '1', '2019-05-24 02:48:21', '2019-06-10 17:59:47', '1', '0', '0', 'GatewayServiceClient', 'getApiWhiteList');
...
(
  ...
  app_name_en  varchar2(255) not null,
  app_icon     varchar2(255),
  app_type     varchar2(50) not null,
  app_desc     varchar2(255) not null,
  app_os      varchar2(25) ,
...
comment on table base_app
  is '系统应用-基础信息';
-- Add comments to the columns
comment on column base_app.app_id
  is '客户端ID';
comment on column base_app.app_secret
  is '客户端秘钥';
comment on column base_app.app_name
  is 'app名称';
comment on column base_app.app_name_en
  is 'app英文名称';
comment on column base_app.app_icon
  is '应用图标';
comment on column base_app.app_type
  is 'app类型:server-服务应用 app-手机应用 pc-PC网页应用 wap-手机网页应用';
comment on column base_app.app_desc
  is 'app描述';
comment on column base_app.app_os
  is '移动应用操作系统:ios-苹果 android-安卓';
comment on column base_app.website
  is '官网地址';
comment on column base_app.user_id
  is '用户ID:默认为0';
comment on column base_app.user_type
  is '用户类型:platform-平台 isp-服务提供商 dev-自研开发者';
comment on column base_app.create_time
  is '创建时间';
comment on column base_app.update_time
  is '更新时间';
comment on column base_app.status
  is '状态:0-无效 1-有效';
comment on column base_app.is_persist
  is '保留数据0-否 1-是 不允许删除';
-- Create/Recreate primary, unique and foreign key constraints
alter table base_app
  add constraint pk_base_app_id primary key (APP_ID);


INSERT INTO base_app
VALUES
  ('1552274783265',
   '2cde1eaa60fe4af1987f94caa13f29a2',
   '资源服务器',
   'ResourceServer',
   '',
   'server',
   ...
   '521677655146233856',
   'platform',
   to_date('2018-11-12 17:48:45','yyyy-mm-dd hh24:mi:ss'),
   to_date('2019-03-16 00:29:42','yyyy-mm-dd hh24:mi:ss'),
   '1',
   '1');
INSERT INTO base_app
VALUES
  ('1552294656514',
   '74a02bade18a42388c3127751b96e1f7',
   '运营后台',
   'Admin',
   '',
   'pc',
   ...
   'platform',
   to_date('2018-11-12 17:48:45','yyyy-mm-dd hh24:mi:ss'),
   to_date('2019-03-16 00:24:26','yyyy-mm-dd hh24:mi:ss'),
   '1',
   '1');
...
create index idx_base_auth_menu_id on base_authority(menu_id);
create index idx_base_auth_api_id on base_authority(api_id);
create index idx_base_auth_action_id on base_authority(action_id);
-- Add comments to the table
comment on table base_authority
  is '系统权限-菜单权限、操作权限、API权限';
-- Add comments to the columns
comment on column base_authority.authority
  is '权限标识';
comment on column base_authority.menu_id
  is '菜单资源ID';
comment on column base_authority.api_id
  is 'API资源ID';
comment on column base_authority.status
  is '状态';
-- ----------------------------
...
--DROP TABLE base_authority_action;
CREATE TABLE base_authority_action (
         action_id number(20) NOT NULL ,--COMMENT '操作ID',
         authority_id number(20) NOT NULL --COMMENT 'API',
);
create index idx_base_auth_act_action_id on base_authority_action(action_id);
create index idx_base_auth_act_authority_id on base_authority_action(authority_id);
-- Add comments to the table
comment on table base_authority_action
  is '系统权限-功能操作关联表';
-- Add comments to the columns
comment on column base_authority_action.action_id
  is '操作ID';
comment on column base_authority_action.authority_id
  is 'API';
-- ----------------------------
...
CREATE index idx_base_authority_app_auth_id on base_authority_app(authority_id);
CREATE index idx_base_authority_app_id on base_authority_app(app_id);
-- Add comments to the table
comment on table base_authority_app
  is '系统权限-应用关联';
-- Add comments to the columns
comment on column base_authority_app.authority_id
  is '权限ID';
-- ----------------------------
...
-- ----------------------------
--DROP TABLE base_authority_role;
...
CREATE index idx_authority_role_auth_id on base_authority_role(authority_id);
CREATE index idx_authority_role_role_id on base_authority_role(role_id);
-- Add comments to the table
comment on table base_authority_role
  is '系统权限-角色关联';
-- Add comments to the columns
comment on column base_authority_role.authority_id
  is '权限ID';
comment on column base_authority_role.role_id
  is '角色ID';
comment on column base_authority_role.expire_time
  is '过期时间:null表示长期';
...
(
  authority_id number(20) NOT NULL ,--COMMENT '权限ID',
   user_id number(20) NOT NULL ,--COMMENT '用户ID',
   expire_time date DEFAULT NULL --COMMENT '过期时间',
);
CREATE index idx_authority_user_auth_id on base_authority_user(authority_id);
CREATE index idx_authority_user_user_id on base_authority_user(user_id);
-- Add comments to the table
comment on table base_authority_user
  is '系统权限-用户关联';
-- Add comments to the columns
comment on column base_authority_user.authority_id
  is '权限ID';
comment on column base_authority_user.user_id
  is '用户ID';
comment on column base_authority_user.expire_time
  is '过期时间';


create sequence base_menu_seq;
--drop table base_menu;
create table base_menu
(
 menu_id number(20) NOT NULL ,--COMMENT '菜单Id',
 parent_id number(20) DEFAULT NULL ,--COMMENT '父级菜单',
 menu_code varchar2(100)  NOT NULL ,--COMMENT '菜单编码',
 menu_name varchar2(100)  NOT NULL ,--COMMENT '菜单名称',
 menu_desc varchar2(100)  DEFAULT NULL ,--COMMENT '描述',
 scheme varchar2(20)  NOT NULL ,--COMMENT '路径前缀',
 path varchar2(255)  DEFAULT NULL ,--COMMENT '请求路径',
 icon varchar2(255)  DEFAULT '' ,--COMMENT '菜单标题',
 target varchar2(20)  DEFAULT '_self' NOT NULL ,--COMMENT '打开方式:_self窗口内,_blank新窗口',
 priority number(20) DEFAULT '0' NOT NULL  ,--COMMENT '优先级 越小越靠前',
 status number(3) DEFAULT '1' NOT NULL  ,--COMMENT '状态:0-无效 1-有效',
 create_time date NOT NULL ,--COMMENT '创建时间',
 update_time date DEFAULT NULL ,--COMMENT '更新时间',
 is_persist number(3) DEFAULT '0' NOT NULL  ,--COMMENT '保留数据0-否 1-是 不允许删除',
 service_id varchar2(100)  NOT NULL ,--COMMENT '服务名',
 PRIMARY KEY (menu_id),
 CONSTRAINT uniq_base_menu_code UNIQUE (menu_code)
);
CREATE index idx_base_menu_service_id on base_menu(service_id);
-- Add comments to the table
comment on table base_menu
  is '系统资源-菜单信息';
-- Add comments to the columns
comment on column base_menu.menu_id
  is '菜单Id';
comment on column base_menu.parent_id
  is '父级菜单';
comment on column base_menu.menu_code
  is '菜单编码';
comment on column base_menu.menu_name
  is '菜单名称';
comment on column base_menu.menu_desc
  is '描述';
comment on column base_menu.scheme
  is '路径前缀';
comment on column base_menu.path
  is '请求路径';
comment on column base_menu.icon
  is '菜单标题';
comment on column base_menu.target
  is '打开方式:_self窗口内,_blank新窗口';
comment on column base_menu.priority
  is '优先级 越小越靠前';
comment on column base_menu.status
  is '状态:0-无效 1-有效';
comment on column base_menu.create_time
  is '创建时间';
comment on column base_menu.update_time
  is '更新时间';
comment on column base_menu.is_persist
  is '保留数据0-否 1-是 不允许删除';
comment on column base_menu.service_id
  is '服务名';
-- ----------------------------
...
(
  ...
  is_persist  number(3) default 0
)
;
-- Add comments to the table
comment on table base_role
  is '系统角色-基础信息';
-- Add comments to the columns
comment on column base_role.role_id
  is '角色ID';
comment on column base_role.role_code
  is '角色编码';
comment on column base_role.role_name
  is '角色名称';
comment on column base_role.status
  is '状态:0-无效 1-有效';
comment on column base_role.role_desc
  is '角色描述';
comment on column base_role.is_persist
  is '保留数据0-否 1-是 不允许删除';
-- Create/Recreate primary, unique and foreign key constraints
alter table base_role
  add constraint pk_base_role_id primary key (ROLE_ID);
alter table base_role
  add constraint unq_role_code unique (ROLE_CODE);
...
(
  authority_id number(18) not null,
  role_id      number(18) not null,
  expire_time  timestamp
)
;
-- Add comments to the table
comment on table base_role_authority
  is '系统角色-授权';
-- Add comments to the columns
comment on column base_role_authority.authority_id
  is '权限ID';
comment on column base_role_authority.role_id
  is '角色ID';
comment on column base_role_authority.expire_time
  is '过期时间:null表示长期';
-- Create/Recreate indexes
create index idx_base_role_auth_auth_id on base_role_authority (authority_id);
create index idx_base_role_auth_role_id on base_role_authority (role_id);
...
;
-- Add comments to the table
comment on table base_role_user
  is '系统角色-角色与用户关联';
-- Add comments to the columns
comment on column base_role_user.user_id
  is '用户ID';
comment on column base_role_user.role_id
  is '角色ID';
-- Create/Recreate indexes
create index idx_base_ur_user on base_role_user (user_id);
create index idx_base_ur_role on base_role_user (role_id);
...
create sequence base_tentant_seq;
CREATE TABLE base_tentant (
      tentant_id number(20) NOT NULL ,--COMMENT '租户ID',
      tentant_name varchar2(100) NOT NULL ,--COMMENT '租户名称',
      tentant_desc varchar2(255) NOT NULL ,--COMMENT '租户描述',
      create_time date NOT NULL ,--COMMENT '创建时间',
      update_time date NOT NULL ,--COMMENT '更新时间',
      PRIMARY KEY (tentant_id)
);
comment on table base_tentant
  is '租户信息表';
-- Add comments to the columns
comment on column base_tentant.tentant_id
  is '租户ID';
comment on column base_tentant.tentant_name
  is '租户名称';
comment on column base_tentant.tentant_desc
  is '租户描述';
comment on column base_tentant.create_time
  is '创建时间';
comment on column base_tentant.update_time
  is '更新时间';


create sequence base_tentant_modules_seq;
CREATE TABLE base_tentant_modules (
        module_id number(20) NOT NULL ,--COMMENT '模块ID',
        tentant_id number(20) NOT NULL ,--COMMENT '租户ID',
        service_id varchar2(100) NOT NULL ,--COMMENT '服务名称',
        module_desc varchar2(255) NOT NULL ,--COMMENT '模块描述',
        is_persist number(3) DEFAULT '0' NOT NULL  ,--COMMENT '保留数据0-否 1-是 不允许删除',
        PRIMARY KEY (module_id)
);
comment on table base_tentant_modules
  is '租户模块';
-- Add comments to the columns
comment on column base_tentant_modules.module_id
  is '模块ID';
comment on column base_tentant_modules.tentant_id
  is '租户ID';
comment on column base_tentant_modules.service_id
  is '服务名称';
comment on column base_tentant_modules.module_desc
  is '模块描述';
comment on column base_tentant_modules.is_persist
  is '保留数据0-否 1-是 不允许删除';
...
create table base_user(
 ...
 email varchar2(100),
 mobile varchar2(50),
 user_type varchar2(20) default 'platform',
 company_id number(18),
 register_ip varchar2(100),
...
comment on table base_user
  is '系统用户-基础信息';
-- Add comments to the columns
comment on column base_user.user_id
  is '用户ID';
comment on column base_user.user_name
  is '登陆账号';
comment on column base_user.nick_name
  is '昵称';
comment on column base_user.avatar
  is '头像';
comment on column base_user.email
  is '邮箱';
comment on column base_user.mobile
  is '手机号';
comment on column base_user.user_type
  is '用户类型:platform-平台 isp-服务提供商 dev-自研开发者';
comment on column base_user.company_id
  is '企业ID';
comment on column base_user.register_ip
  is '注册IP';
comment on column base_user.register_time
  is '注册时间';
comment on column base_user.status
  is '状态:0-禁用 1-启用 2-锁定';
comment on column base_user.user_desc
  is '描述';
comment on column base_user.create_time
  is '创建时间';
comment on column base_user.update_time
  is '更新时间';
-- Drop primary, unique and foreign key constraints
-- Create/Recreate primary, unique and foreign key constraints
alter table base_user
  add constraint pk_base_user_userid primary key (USER_ID);


INSERT INTO base_user VALUES ('521677655146233856', 'admin', '超级管理员', '', '515608851@qq.com', '18518226890', 'platform', null, null, to_date('2018-12-10 13:20:45','yyyy-MM-dd hh24:mi:ss'), '1', '拥有所有权限,不受任何限制.仅限admin账号。', to_date('2018-12-10 13:20:45','yyyy-MM-dd hh24:mi:ss'), to_date('2018-12-10 13:20:45','yyyy-MM-dd hh24:mi:ss'));
INSERT INTO base_user VALUES ('557063237640650752', 'test', '测试用户', '', '', '', 'platform', null, null, to_date('2019-03-18 04:50:25','yyyy-MM-dd hh24:mi:ss'), '1', '', to_date('2019-03-18 04:50:25','yyyy-MM-dd hh24:mi:ss'), to_date('2019-03-18 04:50:25','yyyy-MM-dd hh24:mi:ss'));


--DROP TABLE base_user_account;
create sequence BASE_USER_ACCOUNT_SEQ;
...
create index idx_base_user_account_uid on base_user_account(user_id);
-- Add comments to the table
comment on table BASE_USER_ACCOUNT
  is '系统用户-登录账号';
-- Add comments to the columns
comment on column BASE_USER_ACCOUNT.user_id
  is '用户Id';
comment on column BASE_USER_ACCOUNT.account
  is '标识：手机号、邮箱、 用户名、或第三方应用的唯一标识';
comment on column BASE_USER_ACCOUNT.password
  is '密码凭证：站内的保存密码、站外的不保存或保存token）';
comment on column BASE_USER_ACCOUNT.account_type
  is '登录类型:password-密码、mobile-手机号、email-邮箱、weixin-微信、weibo-微博、qq-等等';
-- Create/Recreate indexes
-- Create/Recreate primary, unique and foreign key constraints
...
create sequence BASE_USER_ACCOUNT_LOGS_SEQ;
-- Create table
...
(
  ...
  login_agent  VARCHAR2(500) not null,
  login_nums   NUMBER(11) not null,
  user_id      NUMBER(20) not null,
...
-- Add comments to the table
comment on table BASE_USER_ACCOUNT_LOGS
  is '系统用户-登录日志';
-- Add comments to the columns
comment on column BASE_USER_ACCOUNT_LOGS.login_ip
  is '登录Ip';
comment on column BASE_USER_ACCOUNT_LOGS.login_agent
  is '登录设备';
comment on column BASE_USER_ACCOUNT_LOGS.login_nums
  is '登录次数';
comment on column BASE_USER_ACCOUNT_LOGS.account_id
  is '账号ID';
-- Create/Recreate primary, unique and foreign key constraints
alter table BASE_USER_ACCOUNT_LOGS
  add constraint PK_BASE_USER_ACCOUNT_LOGS primary key (ID);


-- Create table
create table base_user_authority
(
  authority_id number(18),
  user_id      number(18),
  expire_time  timestamp
)
;
-- Add comments to the table
comment on table base_user_authority
  is '系统用户-授权';
-- Create/Recreate indexes
create index idx_base_ua_authid on base_user_authority (authority_id);
create index idx_base_ua_uid on base_user_authority (user_id);
